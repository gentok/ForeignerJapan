#' ---
#' title: "SIFCCT Recoding Ver 5 (Additional Recode)"
...
#+ echo=FALSE
dzip$zip_did <- readRDS(paste0(projdir,"/data/sifcct_zip_did_v5.rds"))
#+ 
...
## Drop Cases with incomplete ZIPs
dzip$zip[which(substr(dzip$zip,6,7)%in%"00")] <- NA
dzip <- dzip[complete.cases(dzip$zip),]
...
# Exporting HTML File
# In R Studio
# rmarkdown::render('./src/data_sifcct_1x_recode_v5.R', rmarkdown::pdf_document(latex_engine="xelatex", extra_dependencies = list(bookmark=NULL, xltxtra=NULL, zxjatype=NULL, zxjafont=c("ipa"))), encoding = 'UTF-8')
# rmarkdown::render('./src/data_sifcct_1x_recode_v5.R', 'github_document', clean=FALSE)
